...
class PieAccessibilityHandler {

    private final static String TAG = PieAccessibilityHandler.class.getSimpleName();
        ...
        switch (eventType) {
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                if (isPermissionsDialog(source)) {
                    ...
                    Log.d(TAG, "We are in App info screen.");
                    ...
                    Log.d(TAG, "We are in App permissions (list of all permission) in window state change.");
                    insideSinglePermissionToAllAppsSettingsScreen = false;
            ...
            case AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED:
                if (insideSettingsAppPermissionsForAnAppScreen) {
                    Log.d(TAG, "Window content changed in app to permission settings screen.");
                    if (permissionNames2permissionSwitchStatus != null && !permissionNames2permissionSwitchStatus.isEmpty()) {
                        Log.d(TAG, "permissionNames2permissionSwitchStatus not empty, finding difference...");
                        findDifferenceBetweenPermissionNameToSwitchStatusAndSendEvents(source);
                    }
                } else if (insideSinglePermissionToAllAppsSettingsScreen) {
                    Log.d(TAG, "Window content changed in permission to app list screen.");
                    if (appNames2PermissionSwitchStatus != null && !appNames2PermissionSwitchStatus.isEmpty()) {
                        Log.d(TAG, "appNames2PermissionSwitchStatus not empty, finding difference...");
                        findDifferenceBetweenAppNameToSwitchStatusAndSendEvents(source);
                    }
                    // NOTE: When window state changed event happens, UI may not have the list of apps to permission status ready because of the loading screen. So we need to log the current permission settings again.
                    extractAppNameToSinglePermissionSwitchStatus(source);
                } else if (isAppProactivePermissionRequest(source)) {
                    Log.d(TAG, "We encountered an app proactive permission dialog.");
                    runIntoAppProactivePermissionRequestDialog = true;
    ...
    //region Permission to app list related
